    ...
    <div className="page" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "var(--page-px)", background: "var(--bg)" }}>
      <div style={{ position: "relative", maxWidth: 440, width: "100%" }}>
        ...
        <div className="card" style={{ padding: "32px 28px", textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ fontFamily: "var(--font-hand), Caveat, cursive", fontSize: 48, fontWeight: 700, color: "#EF4444", lineHeight: 1, marginTop: 8 }}>Oops</div>
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 8, lineHeight: 1.5 }}>Something went wrong loading brands.</div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 20, fontFamily: "var(--font-mono)" }}>{error.message?.length > 80 ? error.message.slice(0, 80) + "..." : error.message}</div>

          <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
            <button onClick={reset} className="btn btn-primary">Try again</button>
            <Link href="/brands" className="btn btn-ghost" style={{ textDecoration: "none" }}>Back to brands</Link>
          </div>
        </div>
      </div>
    </div>
