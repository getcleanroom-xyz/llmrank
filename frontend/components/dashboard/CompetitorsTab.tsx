    ...
    <div>
      <div style={{ marginBottom: 18 }}>
        ...
        <svg width="160" height="6" viewBox="0 0 160 6" preserveAspectRatio="none" style={{ display: "block" }}>
          <path d="M0 3 Q10 0 20 5 Q30 8 40 3 Q50 0 60 6 Q70 8 80 2 Q90 0 100 5 Q110 8 120 3 Q130 0 140 5 Q150 8 160 3" fill="none" stroke="#A855F7" strokeWidth="2" strokeLinecap="round" />
        </svg>
      </div>
          ...
          {[{ name: brandName, pct: mentionRate, mine: true }, ...competitorShare.map((c) => ({ name: c.name, pct: c.mention_pct, mine: false }))].sort((a, b) => b.pct - a.pct).map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div className="bar-track" style={{ flex: 1, height: 10, background: item.mine ? undefined : "var(--bg-dark)" }}>
                <div className="bar-fill" style={{ width: `${(item.pct / (maxPct || 1)) * 100}%`, background: item.mine ? "var(--primary)" : "#A855F7", borderRadius: 0 }} />
              </div>
      ...
      {competitorShare.length > 0 && (
        <div className="card" style={{ padding: "14px 18px", marginBottom: "var(--gap)", borderColor: "var(--primary)", transform: "rotate(0.15deg)", background: "#FFF9DB" }}>
          ...
          {competitorShare.slice(0, 3).map((c, i) => (
            <div key={c.name} style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.5, marginBottom: i < 2 ? 6 : 0, paddingLeft: 12, borderLeft: "2px solid var(--primary)" }}>
      ...
      <div className="grid-2">
        {competitorShare.map((c, i) => (
          <div
            ...
            style={{ padding: "14px 16px", cursor: "pointer", transform: `rotate(${i % 2 === 0 ? "-0.2deg" : "0.2deg"})`, borderTop: "4px solid #A855F7", transition: "box-shadow 0.15s, transform 0.15s" }}
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div className="bar-track" style={{ flex: 1, height: 8 }}>
                <div className="bar-fill" style={{ width: `${(c.mention_pct / (maxPct || 1)) * 100}%`, background: "#A855F7", borderRadius: 0 }} />
              </div>
              <span style={{ fontSize: 13, fontWeight: 800 }}>{c.mention_pct}%</span>
            </div>
            <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4, fontWeight: 600 }}>Click for detail</div>
          </div>
        ))}
      </div>
    </div>
