...
export function QueryChipsPanel({ queries, brandId, onManageQueries }: { queries: QuerySummary[]; brandId: string; onManageQueries?: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {queries.map((q) => (
        <div key={q.query_id} role="button" tabIndex={0} onClick={() => router.push(`/brands/${brandId}/queries/${q.query_id}`)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); router.push(`/brands/${brandId}/queries/${q.query_id}`); } }} className="card sketchy" style={{ background: "var(--bg-dark)", border: "2px solid var(--border)", borderRadius: "var(--radius)", padding: "10px 12px", cursor: "pointer", transition: "box-shadow 0.1s, transform 0.1s" }} onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "var(--shadow-hover)"; e.currentTarget.style.transform = "translate(-1px, -1px)"; }} onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "2px 2px 0 #1A1A1A, 3px 3px 0 #1A1A1A"; e.currentTarget.style.transform = "none"; }}>
