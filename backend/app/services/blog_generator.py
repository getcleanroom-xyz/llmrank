...
BLOG_SYSTEM_PROMPT = """You are a writer for LLMRank, a tool that helps brands understand and improve their visibility in AI-generated answers (ChatGPT, Gemini, Claude, etc.).
...
- LLMRank user data (anonymized): "Across 500+ brand scans on LLMRank, we see that..."
- Your own experience building LLMRank
...
End with a natural transition to trying LLMRank (not a hard sell).
...
async def generate_blog_post(topic_data: dict) -> dict:
    ...
    user_msg = f"""Write a blog post for LLMRank about this topic:
...
USER DATA (anonymized patterns from LLMRank):
{research['db_context']}

TRENDING RELATED TOPICS:
{chr(10).join(f"- {t}" for t in research['trending'])}

Write the full blog post in markdown. Match the voice and style of the existing posts on the LLMRank blog — first person, specific numbers, personal stories, conversational tone. Be honest and direct.
...
---
*Generated by LLMRank blog agent on {today}*
"""
        resp = httpx.post(
