...
async def _check_and_increment_rate_limit(db: AsyncSession, user_id: uuid.UUID) -> bool:
    """Atomically check and increment hourly rate limit. Returns True if allowed."""
    bucket = _current_hour_bucket()
    result = await db.execute(
        select(AgentRateLimit)
        .where(AgentRateLimit.user_id == user_id, AgentRateLimit.hour_bucket == bucket)
        .with_for_update()
    )
    limit = result.scalar_one_or_none()
    max_per_hour = 20
    if limit:
        if limit.request_count >= max_per_hour:
            return False
...
async def recommend(
    ...
    # Rate limit check
    if not await _check_and_increment_rate_limit(db, user.id):
...
async def recommend_stream(
    ...
    async def generate():
        ...
        try:
            ...
            logger.error("Stream error for brand %s: %s", brand_id, e)
        ...
        if full_response:
            try:
                ...
                logger.warning("Failed to publish chat messages: %s", e)
